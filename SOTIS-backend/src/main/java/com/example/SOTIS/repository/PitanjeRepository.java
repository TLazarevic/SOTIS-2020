...
public interface PitanjeRepository extends JpaRepository<Pitanje, Long> {
	...
	// @Query("SELECT new com.example.SOTIS.model.DTO.TestDTO(t, n, ut.uradjen) from
	// UcenikTest ut join ut.test t join t.predmet p join p.ucenik u join
	// t.nastavnik n WHERE u.id=?1")
	...
	public List<Pitanje> findByTest(Test test);

	public List<Pitanje> findByTestId(Long testId);
